...
class CairngormsSpider(scrapy.Spider):
    name = 'Cairngorms'
    ...
    def start_requests(self):
        ...
        ii = ['https://www.eplanningcnpa.co.uk/online-applications/search.do?action=property&type=atoz']
        ...
        self.planning = db.Cairngorms_Council
        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Cairngorms Connected successfully')
                sts = True
            except:
                count += 1
