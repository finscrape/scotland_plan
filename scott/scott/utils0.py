...
def cookies_parseeal():
    cookie_string = 'JSESSIONID=Z7SW7LOBKqZxdeCTT0sFzl4W4bOV0QSuk9GY-lN_.cotton'
    cookie = SimpleCookie()
    cookie.load(cookie_string)
    parsed_cookie = {}
    for key,morsel in cookie.items():
...
def cookies_parseear():
    cookie_string = 'JSESSIONID=2CMV92s9uROuL52n4sj3xDCpUC6YC3NVHCD1cZHN.idoxweb-live'
    cookie = SimpleCookie()
    cookie.load(cookie_string)
    parsed_cookie = {}
    for key,morsel in cookie.items():
