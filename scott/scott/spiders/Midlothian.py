...
from scott.utils0 import cookies_parsep


class MidlothianSpider(scrapy.Spider):
    name = 'Midlothian'
    ...
    def start_requests(self):
        ...
        ii = ['https://planning-applications.midlothian.gov.uk/OnlinePlanning/simpleSearchResults.do?action=firstPage']
        ...
        self.planning = db.Midlothian_Council

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Midlothian_Council Connected successfully')
                sts = True
            except:
                count += 1
        ...
        for i in numb:
            ...
            ii = ['https://planning-applications.midlothian.gov.uk/OnlinePlanning/simpleSearchResults.do?action=firstPage']
            head = 'https://planning-applications.midlothian.gov.uk'
            
            pay = f'_csrf=4489f889-f3e7-4ca8-b920-2eb479dc7173&searchType=Application&searchCriteria.caseStatus=&searchCriteria.simpleSearchString={i}&searchCriteria.simpleSearch=true'
            headers = {
                ...
                'Host': 'planning-applications.midlothian.gov.uk',
                'Origin': 'https://planning-applications.midlothian.gov.uk',
                'Pragma': 'no-cache',
                'Referer': 'https://planning-applications.midlothian.gov.uk/OnlinePlanning/search.do?action=simple'
            }

            yield scrapy.Request(url=ii[0],method='POST',body =pay,headers=headers,cookies=cookies_parsep(),meta={'h':head},dont_filter=True)
