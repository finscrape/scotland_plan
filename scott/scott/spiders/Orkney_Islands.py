...
class OrkneyIslandsSpider(scrapy.Spider):
    name = 'Orkney_Islands'
    ...
    def start_requests(self):
        ...
        ii = ['https://planningandwarrant.orkney.gov.uk/online-applications/search.do?action=property&type=atoz']
        ...
        self.planning = db.Orkney_Islands_Council

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Orkney_Islands_Council Connected successfully')
                sts = True
            except:
                count += 1
