...
from scott.utils0 import cookies_parsesol
...
class SouthLanarkshireSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        ii = ['https://publicaccess.southlanarkshire.gov.uk/online-applications/propertySearchResults.do?action=firstPage']
        ...
        self.planning = db.South_Lanarkshire_Councils

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('South_Lanarkshire_Council Connected successfully')
                sts = True
            except:
                count += 1
