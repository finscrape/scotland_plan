...
class WestLothianSpider(scrapy.Spider):
    name = 'West_Lothian'
    ...
    def start_requests(self):
        ...
        ii = ['https://planning.westlothian.gov.uk/publicaccess/search.do?action=property&type=atoz']
        ...
        self.planning = db.West_Lothian_Council

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('West_Lothian_Council Connected successfully')
                sts = True
            except:
                count += 1
