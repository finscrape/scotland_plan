...
from scott.utils0 import cookies_parsesti
...
class StirlingSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        ii = ['https://pabs.stirling.gov.uk/online-applications/propertySearchResults.do?action=firstPage']
        ...
        self.planning = db.Stirling_Councils

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Stirling_Council Connected successfully')
                sts = True
            except:
                count += 1
        ...
        for i in ii[0:]:
            ...
            for p in pcode:
                    ...
                    tx = f'_csrf=721268bd-6a03-4a7f-9849-c880dae1616a&searchCriteria.uprn=&searchCriteria.propertyNameNumber=&searchCriteria.streetName=&searchCriteria.locality=&searchCriteria.town=&searchCriteria.postCode={p}&searchType=Property'
                    yield scrapy.Request(url=i,meta={'f':i,'p':p,'h':h},dont_filter= True,callback=self.parse,cookies=cookies_parsesti(),method='POST',body=tx)
