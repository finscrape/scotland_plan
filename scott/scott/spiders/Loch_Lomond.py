...
class LochLomondSpider(scrapy.Spider):
    name = 'Loch_Lomond'
    ...
    def start_requests(self):
        ...
        ii = ['https://eplanning.lochlomond-trossachs.org/OnlinePlanning/search.do?action=property&type=atoz']
        #connecting to mongo
        URI = 'mongodb://0.tcp.eu.ngrok.io:17907'
        client = MongoClient(URI)

        db = client.get_database('scotland')
        self.planning = db.Loch_Lomond_Council

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Loch_Lomond_Council Connected successfully')
                sts = True
            except:
                count += 1
