...
from scott.utils0 import cookies_parsed
...
class ClackmannanshireSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        self.planning = db.Clackmannanshire_Council
        count = 0
        while True:
            try:
        ...
        for i in numb:
            ii = ['https://publicaccess.clacks.gov.uk/publicaccess/simpleSearchResults.do?action=firstPage']
            head = 'https://publicaccess.clacks.gov.uk'
            
            pay = f'_csrf=6fb0d0d7-9c4e-413a-9341-692b8ea10c9c&searchType=Application&searchCriteria.caseStatus=&searchCriteria.simpleSearchString={i}&searchCriteria.simpleSearch=true'
            headers = {
                ...
                'Host': 'publicaccess.clacks.gov.uk',
                'Origin': 'https://publicaccess.clacks.gov.uk',
                'Pragma': 'no-cache',
                'Referer': 'https://publicaccess.clacks.gov.uk/publicaccess/search.do?action=simple'

            }

            yield scrapy.Request(url=ii[0],method='POST',body =pay,headers=headers,cookies=cookies_parsed(),meta={'h':head},dont_filter=True)
