...
from scott.utils0 import cookies_parsenoa
...
class NorthAyrshireSpider(scrapy.Spider):
    name = 'North_Ayrshire'
    ...
    def start_requests(self):
        ...
        ii = ['https://www.eplanning.north-ayrshire.gov.uk/OnlinePlanning/propertySearchResults.do?action=firstPage']
        ...
        self.planning = db.North_Ayrshire_Councils

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('North_Ayrshire_Council Connected successfully')
                sts = True
            except:
                count += 1
        ...
        for i in ii[0:]:
            ...
            for p in pcode[50000:100000]:
                    ...
                    tx = f'_csrf=627e5ae7-32ab-46ed-b830-81455f3a40f5&searchCriteria.uprn=&searchCriteria.propertyNameNumber=&searchCriteria.streetName=&searchCriteria.locality=&searchCriteria.town=&searchCriteria.postCode={p}&searchType=Property'
                    yield scrapy.Request(url=i,meta={'f':i,'p':p,'h':h},dont_filter= True,callback=self.parse,cookies=cookies_parsenoa(),method='POST',body=tx)
