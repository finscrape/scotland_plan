...
from scott.utils0 import cookies_parseref
...
class RenfrewshireSpider(scrapy.Spider):
    name = 'Renfrewshire'
    ...
    def start_requests(self):
        ...
        ii = ['https://pl-bs.renfrewshire.gov.uk/online-applications/propertySearchResults.do?action=firstPage']
        ...
        self.planning = db.Renfrewshire_Councils

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Renfrewshire_Council Connected successfully')
                sts = True
            except:
                count += 1
        ...
        for i in ii[0:]:
            ...
            for p in pcode[50000:100000]:
                    ...
                    tx = f'_csrf=40862ab9-78d1-4589-84d5-da0cee0a1ff0&searchCriteria.uprn=&searchCriteria.propertyNameNumber=&searchCriteria.streetName=&searchCriteria.locality=&searchCriteria.town=&searchCriteria.postCode={p}&searchType=Property'
                    yield scrapy.Request(url=i,meta={'f':i,'p':p,'h':h},dont_filter= True,callback=self.parse,cookies=cookies_parseref(),method='POST',body=tx)
