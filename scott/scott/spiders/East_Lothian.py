...
from scott.utils0 import cookies_parseh
...
class EastLothianSpider(scrapy.Spider):
    name = 'East_Lothian'
    ...
    def start_requests(self):
        ...
        ii = ['https://pa.eastlothian.gov.uk/online-applications/simpleSearchResults.do?action=firstPage']
        ...
        self.planning = db.East_Lothian_Council
        
        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('East lothian Connected successfully')
                sts = True
            except:
                count += 1
        ...
        for i in numb:
            ii = ['https://pa.eastlothian.gov.uk/online-applications/simpleSearchResults.do?action=firstPage']
            head = 'https://pa.eastlothian.gov.uk'
            
            pay = f'_csrf=9c0c4d73-2095-485b-a86b-f0a7d076e4be&searchType=Application&searchCriteria.caseStatus=&searchCriteria.simpleSearchString={i}&searchCriteria.simpleSearch=true'
            headers = {
                ...
                'Host': 'pa.eastlothian.gov.uk',
                'Origin': 'https://pa.eastlothian.gov.uk',
                'Pragma': 'no-cache',
                'Referer': 'https://pa.eastlothian.gov.uk/online-applications/search.do?action=simple'

            }

            yield scrapy.Request(url=ii[0],method='POST',body =pay,headers=headers,cookies=cookies_parseh(),meta={'h':head},dont_filter=True)
