...
from scott.utils0 import cookies_parseglg
...
class GlasgowSpider(scrapy.Spider):
    name = 'Glasgow'
    ...
    def start_requests(self):
        ...
        ii = ['https://publicaccess.glasgow.gov.uk/online-applications/propertySearchResults.do?action=firstPage']
        ...
        self.planning = db.Glasgow_City_Councils
        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Glasgow Connected successfully')
                sts = True
            except:
                count += 1
        ...
        for i in ii[0:]:
            ...
            for p in pcode:
                    ...
                    tx = f'_csrf=98175ecf-2ea4-4b74-85e2-75d330d55b97&searchCriteria.uprn=&searchCriteria.propertyNameNumber=&searchCriteria.streetName=&searchCriteria.locality=&searchCriteria.town=&searchCriteria.postCode={p}&searchType=Property'
                    yield scrapy.Request(url=i,meta={'f':i,'p':p,'h':h},dont_filter= True,callback=self.parse,cookies=cookies_parseglg(),method='POST',body=tx)
