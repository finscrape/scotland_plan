...
from scott.utils0 import cookies_parseshi
...
class ShetlandIslandsSpider(scrapy.Spider):
    name = 'Shetland_Islands'
    ...
    def start_requests(self):
        ...
        ii = ['https://pa.shetland.gov.uk/online-applications/propertySearchResults.do?action=firstPage']
        ...
        self.planning = db.Shetland_Islands_Councils

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Shetland_Islands_Council Connected successfully')
                sts = True
            except:
                count += 1
