...
from scott.utils0 import cookies_parsei

from time import sleep


class EastrenfrewshireSpider(scrapy.Spider):
    name = 'EastRenfrewshire'
    ...
    def start_requests(self):
        ...
        ii = ['https://planning.fife.gov.uk/online/simpleSearchResults.do?action=firstPage']
        ...
        self.planning = db.East_Renfrewshire_Council
        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Eastrefrenshire Connected successfully')
                sts = True
            except:
                count += 1
        ...
        for i in numb:
            ii = ['https://planning.fife.gov.uk/online/simpleSearchResults.do?action=firstPage']
            head = 'https://planning.fife.gov.uk'
            
            pay = f'_csrf=4cbf5aef-efa7-4b64-87ec-64966db5cda9&searchType=Application&searchCriteria.caseStatus=&searchCriteria.simpleSearchString={i}&searchCriteria.simpleSearch=true'
            headers = {
                ...
                'Host': 'planning.fife.gov.uk',
                'Origin': 'https://planning.fife.gov.uk',
                'Pragma': 'no-cache',
                'Referer': 'https://planning.fife.gov.uk/online/search.do?action=simple'

            }

            yield scrapy.Request(url=ii[0],method='POST',body =pay,headers=headers,cookies=cookies_parsei(),meta={'h':head},dont_filter=True)
