...
from scott.utils0 import cookies_parsef
...
class DundeeSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        ii = ['https://idoxwam.dundeecity.gov.uk/idoxpa-web/search.do?action=property&type=atoz']
        ...
        self.planning = db.Dundee_City_Council
        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Dundee Connected successfully')
                sts = True
            except:
                count += 1
        ...
        for i in numb:
            ii = ['https://idoxwam.dundeecity.gov.uk/idoxpa-web/simpleSearchResults.do?action=firstPage']
            head = 'https://idoxwam.dundeecity.gov.uk'
            
            pay = f'_csrf=0418732a-db48-40db-87f0-fb5f6e4a1acd&searchType=Application&searchCriteria.caseStatus=&searchCriteria.simpleSearchString={i}&searchCriteria.simpleSearch=true'
            headers = {
                ...
                'Host': 'idoxwam.dundeecity.gov.uk',
                'Origin': 'https://idoxwam.dundeecity.gov.uk',
                'Pragma': 'no-cache',
                'Referer': 'https://idoxwam.dundeecity.gov.uk/idoxpa-web/search.do?action=simple'

            }

            yield scrapy.Request(url=ii[0],method='POST',body =pay,headers=headers,cookies=cookies_parsef(),meta={'h':head},dont_filter=True)
