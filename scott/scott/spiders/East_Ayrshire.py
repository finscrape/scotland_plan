...
from scott.utils0 import cookies_parseg


class EastAyrshireSpider(scrapy.Spider):
    name = 'East_Ayrshire'
    ...
    def start_requests(self):
        ...
        ii = ['https://eplanning.east-ayrshire.gov.uk/online/simpleSearchResults.do?action=firstPage']
        ...
        self.planning = db.East_Ayrshire_Council
        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('East_ayrshire Connected successfully')
                sts = True
            except:
                count += 1
        ...
        for i in numb:
            head = 'https://eplanning.east-ayrshire.gov.uk'
            
            pay = f'_csrf=8716a44c-8292-4445-baec-2281c62fd452&searchType=Application&searchCriteria.caseStatus=&searchCriteria.simpleSearchString={i}&searchCriteria.simpleSearch=true'
            headers = {
                ...
                'Host': 'eplanning.east-ayrshire.gov.uk',
                'Origin': 'https://eplanning.east-ayrshire.gov.uk/',
                'Pragma': 'no-cache',
                'Referer': 'https://eplanning.east-ayrshire.gov.uk/online/search.do?action=simple'

            }

            yield scrapy.Request(url=ii[0],method='POST',body =pay,headers=headers,cookies=cookies_parseg(),meta={'h':head},dont_filter=True)
