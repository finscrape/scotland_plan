...
class SuttonSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        ii = ['https://planningregister.sutton.gov.uk/online-applications/search.do?action=property&type=atoz']

        #mongo connect
        URI = string
        client = MongoClient(URI)
        db = client.get_database('UkCouncils')

        self.planning = db.Sutton_City_Council

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Sutton Connected successfully')
                sts = True
