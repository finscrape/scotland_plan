...
from scott.utils0 import cookies_parsenol
...
class NorthLanarkshireSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        ii = ['https://eplanning.northlanarkshire.gov.uk/online-applications/propertySearchResults.do?action=firstPage']
        ...
        self.planning = db.North_Lanarkshire_Councils

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('North_Lanarkshire_Council Connected successfully')
                sts = True
            except:
                count += 1
        ...
        for i in ii[0:]:
            ...
            for p in pcode[50000:100000]:
                    ...
                    tx = f'_csrf=9da32b40-2ba6-4216-a593-e41faaa64211&searchCriteria.uprn=&searchCriteria.propertyNameNumber=&searchCriteria.streetName=&searchCriteria.locality=&searchCriteria.town=&searchCriteria.postCode={p}&searchType=Property'
                    yield scrapy.Request(url=i,meta={'f':i,'p':p,'h':h},dont_filter= True,callback=self.parse,cookies=cookies_parsenol(),method='POST',body=tx)
