...
from scott.utils0 import cookies_parseq
...
class AngusSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for i in numb:
            ...
            ii = ['https://planning.angus.gov.uk/online-applications/simpleSearchResults.do?action=firstPage']
            head = 'https://planning.angus.gov.uk'
            
            pay = f'_csrf=4efe1a3a-f341-4fa1-a80f-358667e3a7d5&searchType=Application&searchCriteria.caseStatus=&searchCriteria.simpleSearchString={i}&searchCriteria.simpleSearch=true'
            headers = {
                ...
                'Host': 'planning.angus.gov.uk',
                'Origin': 'https://planning.angus.gov.uk',
                'Pragma': 'no-cache',
                'Referer': 'https://planning.angus.gov.uk/online-applications/search.do?action=simple&searchType=Application'

            }

            #yield scrapy.Request(url='https://upa.aberdeenshire.gov.uk/online-applications/',callback=self.parse,dont_filter=True)

            yield scrapy.Request(url=ii[0],method='POST',body =pay,headers=headers,cookies=cookies_parseq(),meta={'h':head},dont_filter=True)
    ...
    def parsed(self, response):
        respons = response.meta['t']

        h = response.meta['h']
        ref = respons.xpath("//th[contains(text(),'Reference')]/following-sibling::td/text()").get()
        next = respons.xpath("(//a[text()='Next']/@href)[1]").get()
        print(next)
        print(ref)
