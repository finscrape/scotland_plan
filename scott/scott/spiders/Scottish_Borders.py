...
from scott.utils0 import cookies_parsesco
...
class ScottishBordersSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        ii = ['https://eplanning.scotborders.gov.uk/online-applications/propertySearchResults.do?action=firstPage']
        ...
        self.planning = db.Scottish_Borders_Council

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Scottish_Borders_Council Connected successfully')
                sts = True
            except:
                count += 1
        ...
        for i in ii[0:]:
            ...
            for p in pcode[50000:100000]:
                    ...
                    tx = f'_csrf=e226bdd3-d2d3-443a-bbb7-87c30692b09e&searchCriteria.propertyNameNumber=&searchCriteria.streetName=&searchCriteria.locality=&searchCriteria.town=&searchCriteria.postCode={p}&searchType=Property'
                    yield scrapy.Request(url=i,meta={'f':i,'p':p,'h':h},dont_filter= True,callback=self.parse,cookies=cookies_parsesco(),method='POST',body=tx)
