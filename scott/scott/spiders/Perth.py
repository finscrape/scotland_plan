...
from scott.utils0 import cookies_parseper
...
class PerthSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        ii = ['https://planningapps.pkc.gov.uk/online-applications/propertySearchResults.do?action=firstPage']
        ...
        self.planning = db.Perth_Councils

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Perth_Council Connected successfully')
                sts = True
            except:
                count += 1
        ...
        for i in ii[0:]:
            ...
            for p in pcode[50000:100000]:
                    ...
                    tx = f'_csrf=a3e54ff9-281c-4d07-8146-d8cc7c683904&searchCriteria.uprn=&searchCriteria.propertyNameNumber=&searchCriteria.streetName=&searchCriteria.locality=&searchCriteria.town=&searchCriteria.postCode={p}&searchType=Property'
                    yield scrapy.Request(url=i,meta={'f':i,'p':p,'h':h},dont_filter= True,callback=self.parse,cookies=cookies_parseper(),method='POST',body=tx)
