...
class AberdeenSpider(scrapy.Spider):
    name = 'Aberdeen'
    ...
    def start_requests(self):
        ...
        ii = ['https://publicaccess.aberdeencity.gov.uk/online-applications/propertySearchResults.do?action=firstPage']
        ...
        self.planning = db.Aberdeen_City_Councils

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Aberdeen Connected successfully')
                sts = True
                ...
                self.planning = db.Aberdeen_City_Council
                sts = False
        ...
        for i in ii[0:]:
            ...
            for p in pcode:
                if 'AB' in p:
                    ...
                    tx = f'org.apache.struts.taglib.html.TOKEN=6a4f3fb01c0fe5c84848be5bb48483dc&_csrf=9215239b-2e96-48b2-89a9-c2b141dedc7f&searchCriteria.uprn=&searchCriteria.propertyNameNumber=&searchCriteria.streetName=&searchCriteria.locality=&searchCriteria.town=&searchCriteria.postCode={p}&searchType=Property'
                    yield scrapy.Request(url=i,meta={'f':i,'p':p,'h':h},dont_filter= True,callback=self.parse,cookies=cookies_parse(),method='POST',body=tx)
