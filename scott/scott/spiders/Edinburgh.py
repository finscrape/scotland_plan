...
from scott.utils0 import cookies_parsej
...
class EdinburghSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        ii = ['https://citydev-portal.edinburgh.gov.uk/idoxpa-web/simpleSearchResults.do?action=firstPage']
        ...
        self.planning = db.Edinburgh_Council
        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Edinburgh Connected successfully')
                sts = True
            except:
                count += 1
        ...
        for i in numb:
            ii = ['https://citydev-portal.edinburgh.gov.uk/idoxpa-web/simpleSearchResults.do?action=firstPage']
            head = 'https://citydev-portal.edinburgh.gov.uk'
            
            pay = f'_csrf=9b08e1ac-c722-4450-b5b3-a961d41a58ac&searchType=Application&searchCriteria.caseStatus=&searchCriteria.simpleSearchString={i}&searchCriteria.simpleSearch=true'
            headers = {
                ...
                'Host': 'citydev-portal.edinburgh.gov.uk',
                'Origin': 'https://citydev-portal.edinburgh.gov.uk',
                'Pragma': 'no-cache',
                'Referer': 'https://citydev-portal.edinburgh.gov.uk/idoxpa-web/search.do?action=simple'

            }

            yield scrapy.Request(url=ii[0],method='POST',body =pay,headers=headers,cookies=cookies_parsej(),meta={'h':head},dont_filter=True)
    ...
    def last(self,response):
        ...
        if attr:
            ...
            status = ''.join(stat)    
            st = status.strip()
            decision = response.xpath("normalize-space(//table[@id='simpleDetailsTable']/tr[6]/td/descendant::text())").get()    
