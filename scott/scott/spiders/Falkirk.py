...
from scott.utils0 import cookies_parsek

from time import sleep

class FalkirkSpider(scrapy.Spider):
    name = 'Falkirk'
    ...
    def start_requests(self):
        ...
        ii = ['https://edevelopment.falkirk.gov.uk/online/simpleSearchResults.do?action=firstPage']
        ...
        self.planning = db.Falkirk_Council
        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Falfirk Connected successfully')
                sts = True
            except:
                count += 1
        ...
        numb = list(range(1,1000000))
        for i in numb:
            self.ind += 1
            print(f'searching....{self.ind}')
            print(f'searching....{self.ind}')
            print(f'searching....{self.ind}')
            print(f'searching....{self.ind}')
            print(f'searching....{self.ind}')
            ii = ['https://edevelopment.falkirk.gov.uk/online/simpleSearchResults.do?action=firstPage']
            head = 'https://edevelopment.falkirk.gov.uk'

            
            pay = f'_csrf=f1ae8858-8db6-477d-a2ee-7ed8d0d4c3b3&searchType=Application&searchCriteria.caseStatus=&searchCriteria.simpleSearchString={i}&searchCriteria.simpleSearch=true'
            headers = {
                ...
                'Host': 'edevelopment.falkirk.gov.uk',
                'Origin': 'https://edevelopment.falkirk.gov.uk',
                'Pragma': 'no-cache',
                'Referer': 'https://edevelopment.falkirk.gov.uk/online/search.do?action=simple&searchType=Application'

            }

            yield scrapy.Request(url=ii[0],method='POST',body =pay,headers=headers,cookies=cookies_parsek(),meta={'h':head},dont_filter=True)
