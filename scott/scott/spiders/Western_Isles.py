...
class WesternIslesSpider(scrapy.Spider):
    name = 'Western_Isles'
    ...
    def start_requests(self):
        ...
        ii = ['https://planning.cne-siar.gov.uk/PublicAccess/search.do?action=property&type=atoz']
        ...
        self.planning = db.Western_Isles_Council

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Western_Isles_Council Connected successfully')
                sts = True
            except:
                count += 1
