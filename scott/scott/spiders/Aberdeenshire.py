...
from scott.utils0 import cookies_parsea


    


class AberdeenshireSpider(scrapy.Spider):
    name = 'Aberdeenshire'
    ...
    def start_requests(self):
        ...
        self.planning = db.Aberdeenshire_Council

        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Aberdeenshire Connected successfully')
                sts = True
            except:
                count += 1
        ...
        numb = list(range(10,1000000))
        
        for i in numb:

            ii = ['https://upa.aberdeenshire.gov.uk/online-applications/simpleSearchResults.do?action=firstPage']
            head = 'https://upa.aberdeenshire.gov.uk'
            
            pay = f'_csrf=94079874-58bd-4c2a-aefe-271193194785&searchType=Application&searchCriteria.caseStatus=&searchCriteria.simpleSearchString={i}&searchCriteria.simpleSearch=true'
            headers = {
                ...
                'Host': 'upa.aberdeenshire.gov.uk',
                'Origin': 'https://upa.aberdeenshire.gov.uk',
                'Pragma': 'no-cache',
                'Referer': 'https://upa.aberdeenshire.gov.uk/online-applications/search.do?action=simple&searchType=Application'

            }

            #yield scrapy.Request(url='https://upa.aberdeenshire.gov.uk/online-applications/',callback=self.parse,dont_filter=True)

            yield scrapy.Request(url=ii[0],method='POST',body =pay,headers=headers,cookies=cookies_parsea(),meta={'h':head},dont_filter=True)
