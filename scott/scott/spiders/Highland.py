...
class HighlandSpider(scrapy.Spider):
    name = 'Highland'
    ...
    def start_requests(self):
        ...
        ii = ['https://wam.highland.gov.uk/wam/search.do?action=property&type=atoz']

        #connecting to mongo
        URI = 'mongodb://0.tcp.eu.ngrok.io:17907'
        client = MongoClient(URI)

        db = client.get_database('scotland')
        self.planning = db.Highland_Council
        count = 0
        while True:
            try:
                self.planning.create_index([('Check', pymongo.ASCENDING)], unique=True)
                print('Highland_Council Connected successfully')
                sts = True
            except:
                count += 1
        ...
        for i in ii:
            ...
            yield SeleniumRequest(url=i,meta={'f':i})
    def parse(self, response):
        ...
        for i in l:
            if 'http' not in i:
                ...
                yield SeleniumRequest(url=i,dont_filter=True,callback=self.each,meta={'h':h})
            else:
                yield SeleniumRequest(url=i,dont_filter=True,callback=self.each,meta={'h':h})
    ...
    def each(self,response):
        ...
        for i in l:
            if 'http' not in i:
                i = f'{h}{i}'
                yield SeleniumRequest(url=i,meta={'h':h},callback=self.street)
            else:
                yield SeleniumRequest(url=i,meta={'h':h},callback=self.street)
        ...
        if next:
            ...
            ab_next = f'{h}{next}'
            yield SeleniumRequest(url=ab_next,callback=self.each,meta={'h':h})
    ...
    def street(self,response):
        ...
        if link:
            for i in link:
                ii = f'{h}{i}'
                yield SeleniumRequest(url=ii,callback=self.pty,meta={'h':h})
        elif urp is not None:
            current = response.url
            yield SeleniumRequest(url=current,callback=self.pty,meta={'h':h},dont_filter=True)
        ...
        if next:
            ab_next = f'{h}{next}'
            yield SeleniumRequest(url=ab_next,callback=self.street,meta={'h':h})
    ...
    def pty(self,response):
        ...
        if link:
            a_link = f'{h}{link}'
            yield SeleniumRequest(url=a_link,meta={'h':h,'u':urpn},callback=self.info)
        ...
        for i in link[0:1]:
            ...
            if li:
                for i in li:
                    if i:
                        a_li = f'{h}{i}'
                        yield SeleniumRequest(url=a_li,meta={'h':h,'t':title,'u':u},callback=self.last)
